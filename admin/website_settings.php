 ...
 * Admin-only control panel for managing the promotional website content.
 */
...
// Content management: trainers shown on the public website
$conn->query("CREATE TABLE IF NOT EXISTS website_trainers (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
// Content management: gym photo gallery shown on the public website
$conn->query("CREATE TABLE IF NOT EXISTS website_gallery (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
foreach ([
    ...
    "ALTER TABLE gym_settings ADD COLUMN IF NOT EXISTS about_image VARCHAR(255) DEFAULT ''",
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Helper: handle a single image upload, returns relative path or '' ────
    if (!function_exists('ws_handle_image_upload')) {
        function ws_handle_image_upload($field, $subdir) {
            if (!isset($_FILES[$field]) || $_FILES[$field]['error'] !== 0) return ['ok' => true, 'path' => ''];
            $allowed_types = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
            $max_size = 5 * 1024 * 1024; // 5MB
            if (!in_array($_FILES[$field]['type'], $allowed_types) || $_FILES[$field]['size'] > $max_size) {
                return ['ok' => false, 'path' => '', 'error' => 'Invalid file type or size. Use JPG, PNG, GIF, or WEBP under 5MB.'];
            }
            $ext = strtolower(pathinfo($_FILES[$field]['name'], PATHINFO_EXTENSION));
            $dir = "../uploads/$subdir/";
            if (!is_dir($dir)) mkdir($dir, 0755, true);
            $fname = $subdir . '_' . time() . '_' . mt_rand(1000, 9999) . '.' . $ext;
            $dest  = $dir . $fname;
            if (!move_uploaded_file($_FILES[$field]['tmp_name'], $dest)) {
                return ['ok' => false, 'path' => '', 'error' => 'Error uploading image file.'];
            }
            return ['ok' => true, 'path' => 'uploads/' . $subdir . '/' . $fname];
        }
    }
    ...
    if (isset($_POST['save_website_info'])) {
        ...
        // Optional About Us photo upload (kept separate from the text fields above)
        $about_up = ws_handle_image_upload('about_image', 'about');
        if (!$about_up['ok']) {
            $message  = $about_up['error'];
            $msg_type = 'danger';
        } else {
            if (!empty($about_up['path'])) {
                // Remove the old about image before saving the new path
                $old = $conn->query("SELECT about_image FROM gym_settings WHERE id = 1")->fetch_assoc();
                if ($old && !empty($old['about_image']) && file_exists('../' . $old['about_image'])) {
                    @unlink('../' . $old['about_image']);
                }
                $sets[]   = "about_image = ?";
                $params[] = $about_up['path'];
                $types   .= 's';
            }
            $params[] = 1; $types .= 'i';
    ...
    if (isset($_POST['delete_booking'])) {
        ...
        $conn->query("DELETE FROM website_bookings WHERE id = $id");
    ...
    if (isset($_POST['update_booking'])) {
        ...
        if ($status === 'verified') {
            ...
            $stmt = $conn->prepare("UPDATE website_bookings SET status = ?, admin_notes = ?, is_read = 1, verified_by = NULL WHERE id = ?");
    ...
    if (isset($_POST['add_trainer'])) {
        ...
        $role = trim($_POST['trainer_role'] ?? '');
        $bio  = trim($_POST['trainer_bio']  ?? '');
        $tags = trim($_POST['trainer_tags'] ?? '');
        if ($name === '') {
            $message = 'Trainer name is required.'; $msg_type = 'danger';
        } else {
            $up = ws_handle_image_upload('trainer_image', 'trainers');
            if (!$up['ok']) {
                $message = $up['error']; $msg_type = 'danger';
            } else {
                $stmt = $conn->prepare("INSERT INTO website_trainers (name, role, bio, tags, image_path) VALUES (?,?,?,?,?)");
                $stmt->bind_param('sssss', $name, $role, $bio, $tags, $up['path']);
                $stmt->execute();
    ...
    if (isset($_POST['toggle_trainer'])) {
        $id  = (int)$_POST['trainer_id'];
        $cur = (int)$_POST['current_active'];
        $conn->query("UPDATE website_trainers SET is_active = " . ($cur ? 0 : 1) . " WHERE id = $id");
        $tab = 'content';
    }
    ...
    if (isset($_POST['delete_trainer'])) {
        $id = (int)$_POST['trainer_id'];
        $row = $conn->query("SELECT image_path FROM website_trainers WHERE id = $id")->fetch_assoc();
        if ($row && !empty($row['image_path']) && file_exists('../' . $row['image_path'])) {
            @unlink('../' . $row['image_path']);
        }
        $conn->query("DELETE FROM website_trainers WHERE id = $id");
    ...
    if (isset($_POST['add_gallery'])) {
        ...
        $up = ws_handle_image_upload('gallery_image', 'gallery');
        if (empty($up['path'])) {
            $message = $up['error'] ?? 'Please choose an image to upload.'; $msg_type = 'danger';
        } else {
            $stmt = $conn->prepare("INSERT INTO website_gallery (image_path, caption) VALUES (?,?)");
            $stmt->bind_param('ss', $up['path'], $caption);
            $stmt->execute();
    ...
    if (isset($_POST['toggle_gallery'])) {
        ...
        $cur = (int)$_POST['current_active'];
        $conn->query("UPDATE website_gallery SET is_active = " . ($cur ? 0 : 1) . " WHERE id = $id");
        $tab = 'content';
    }
    ...
    if (isset($_POST['delete_gallery'])) {
        $id = (int)$_POST['gallery_id'];
        $row = $conn->query("SELECT image_path FROM website_gallery WHERE id = $id")->fetch_assoc();
        if ($row && !empty($row['image_path']) && file_exists('../' . $row['image_path'])) {
            @unlink('../' . $row['image_path']);
        }
        $conn->query("DELETE FROM website_gallery WHERE id = $id");
...
// Backfill verified_by for existing approved bookings that were verified before this column existed
$conn->query("UPDATE website_bookings 
    SET verified_by = (SELECT id FROM users WHERE role='admin' ORDER BY id ASC LIMIT 1)
    WHERE status = 'verified' AND verified_by IS NULL AND created_member_id IS NOT NULL");
...
$trainers = $conn->query("SELECT * FROM website_trainers ORDER BY sort_order, id")->fetch_all(MYSQLI_ASSOC);
...
<div class="container-fluid px-4 pb-5">
  ...
  <ul class="nav nav-tabs mb-4" id="wsTabs">
    ...
    <li class="nav-item">
      <a class="nav-link <?= $tab==='content'?'active':'' ?>" href="?tab=content">
        <i class="fas fa-images me-1"></i> Content (Trainers & Gallery)
      </a>
    </li>
    <li class="nav-item">
        ...
        <div class="col-md-5">
          <form method="POST" id="historyForm-<?= $b['id'] ?>">
            ...
            <div class="d-flex flex-column gap-2">
              ...
              <button type="button"
                class="btn btn-sm btn-danger fw-semibold mt-1"
                onclick="confirmDeleteBooking(<?= $b['id'] ?>, '<?= addslashes(htmlspecialchars($b['full_name'])) ?>')">
          ...
          <!-- Hidden delete form -->
          <form method="POST" id="deleteBookingForm-<?= $b['id'] ?>" style="display:none">
            <input type="hidden" name="booking_id" value="<?= $b['id'] ?>">
  ...
  <!-- ══════════ CONTENT TAB (TRAINERS + GALLERY) ══════════ -->
  <?php elseif ($tab === 'content'): ?>
  <div class="mb-2">
    <p class="text-muted mb-0" style="font-size:.85rem;">Manage the photos, trainers, and other visual content shown on the public website. Changes appear on the site immediately.</p>
  </div>

  <!-- Trainers -->
  <h6 class="fw-bold mt-4 mb-3"><i class="fas fa-user-friends me-2 text-warning"></i>Trainers / Coaches</h6>
  <div class="row g-4">
    <div class="col-md-5">
      <div class="card border-0 shadow-sm">
        <div class="card-body">
          <h6 class="fw-bold mb-3"><i class="fas fa-plus-circle me-2 text-warning"></i>Add Trainer</h6>
          <form method="POST" enctype="multipart/form-data">
            <div class="mb-3">
              ...
              <div class="form-text">JPG, PNG, GIF, or WEBP — under 5MB.</div>
            </div>
            <div class="mb-3">
              <label class="form-label small fw-semibold">Name *</label>
              <input type="text" name="trainer_name" class="form-control" placeholder="e.g. Coach Mike Santos" required>
            </div>
            <div class="mb-3">
              <label class="form-label small fw-semibold">Role / Title</label>
              <input type="text" name="trainer_role" class="form-control" placeholder="e.g. Head Strength Coach">
            </div>
            <div class="mb-3">
              <label class="form-label small fw-semibold">Bio</label>
              <textarea name="trainer_bio" class="form-control" rows="3" placeholder="Short bio / specialties"></textarea>
            </div>
            <div class="mb-3">
              <label class="form-label small fw-semibold">Tags</label>
              <input type="text" name="trainer_tags" class="form-control" placeholder="Comma-separated, e.g. HIIT, Nutrition, Zumba">
            </div>
            <button type="submit" name="add_trainer" class="btn btn-warning w-100 fw-bold">
              <i class="fas fa-plus me-1"></i> Add Trainer
            </button>
          </form>
        </div>
      </div>
    ...
    <div class="col-md-7">
      <h6 class="fw-bold mb-3">Current Trainers <span class="text-muted small fw-normal">(shown on website)</span></h6>
      <?php if (empty($trainers)): ?>
        <div class="text-center py-4 text-muted">No trainers added yet — the website will show default sample trainers until you add real ones.</div>
      <?php else: foreach ($trainers as $t): ?>
        <div class="promo-row bg-white <?= !$t['is_active'] ? 'opacity-50' : '' ?>" style="align-items:center;">
          <div style="width:56px;height:56px;border-radius:12px;overflow:hidden;flex-shrink:0;background:#eee;display:flex;align-items:center;justify-content:center;">
            <?php if (!empty($t['image_path']) && file_exists('../' . $t['image_path'])): ?>
              <img src="../<?= htmlspecialchars($t['image_path']) ?>" style="width:100%;height:100%;object-fit:cover;">
            <?php else: ?>
          ...
          <div class="flex-grow-1 ms-3">
            ...
            <?php if (!$t['is_active']): ?><span class="badge bg-secondary ms-2">Hidden</span><?php endif; ?>
            <div class="small text-muted"><?= htmlspecialchars($t['role']) ?></div>
          </div>
          <div class="d-flex gap-2 flex-shrink-0">
            <form method="POST">
              <input type="hidden" name="trainer_id" value="<?= $t['id'] ?>">
              <input type="hidden" name="current_active" value="<?= $t['is_active'] ?>">
              <button type="submit" name="toggle_trainer" class="btn btn-sm <?= $t['is_active'] ? 'btn-outline-secondary' : 'btn-outline-success' ?>">
                <i class="fas fa-<?= $t['is_active'] ? 'eye-slash' : 'eye' ?>"></i>
              </button>
            </form>
            <form method="POST" onsubmit="return confirm('Remove this trainer?')">
              <input type="hidden" name="trainer_id" value="<?= $t['id'] ?>">
              <button type="submit" name="delete_trainer" class="btn btn-sm btn-outline-danger">
                <i class="fas fa-trash"></i>
              </button>
            </form>
          </div>
  ...
  <!-- Gallery -->
  <h6 class="fw-bold mb-3"><i class="fas fa-images me-2 text-warning"></i>Gym Photo Gallery</h6>
  <div class="row g-4">
    <div class="col-md-5">
      <div class="card border-0 shadow-sm">
        <div class="card-body">
          <h6 class="fw-bold mb-3"><i class="fas fa-plus-circle me-2 text-warning"></i>Add Photo</h6>
          <form method="POST" enctype="multipart/form-data">
            <div class="mb-3">
              <label class="form-label small fw-semibold">Photo *</label>
            ...
            <div class="mb-3">
              <label class="form-label small fw-semibold">Caption (optional)</label>
              <input type="text" name="gallery_caption" class="form-control" placeholder="e.g. Free weights area">
            </div>
            <button type="submit" name="add_gallery" class="btn btn-warning w-100 fw-bold">
    ...
    <div class="col-md-7">
      <h6 class="fw-bold mb-3">Current Photos <span class="text-muted small fw-normal">(shown on website)</span></h6>
      <?php if (empty($gallery)): ?>
        <div class="text-center py-4 text-muted">No photos added yet — the website will show placeholder icons until you add real photos.</div>
      <?php else: ?>
        <div class="row g-3">
        <?php foreach ($gallery as $g): ?>
          <div class="col-6 col-lg-4">
            <div class="card border-0 shadow-sm h-100 <?= !$g['is_active'] ? 'opacity-50' : '' ?>">
              <div style="height:110px;overflow:hidden;background:#eee;">
                <?php if (file_exists('../' . $g['image_path'])): ?>
                  <img src="../<?= htmlspecialchars($g['image_path']) ?>" style="width:100%;height:100%;object-fit:cover;">
                <?php endif; ?>
              </div>
              <div class="card-body p-2">
                <div class="small text-truncate mb-2"><?= htmlspecialchars($g['caption'] ?: 'Untitled') ?></div>
                <div class="d-flex gap-2">
                  <form method="POST" class="flex-fill">
                    <input type="hidden" name="gallery_id" value="<?= $g['id'] ?>">
                    <input type="hidden" name="current_active" value="<?= $g['is_active'] ?>">
                    <button type="submit" name="toggle_gallery" class="btn btn-sm w-100 <?= $g['is_active'] ? 'btn-outline-secondary' : 'btn-outline-success' ?>">
                      <i class="fas fa-<?= $g['is_active'] ? 'eye-slash' : 'eye' ?>"></i>
                    </button>
                  </form>
                  <form method="POST" class="flex-fill" onsubmit="return confirm('Remove this photo?')">
                    <input type="hidden" name="gallery_id" value="<?= $g['id'] ?>">
                    <button type="submit" name="delete_gallery" class="btn btn-sm w-100 btn-outline-danger">
                      <i class="fas fa-trash"></i>
                    </button>
                  </form>
                </div>
        ...
        <div class="card-body p-4">
          ...
          <form method="POST" enctype="multipart/form-data">
            <div class="row g-3">
              ...
              <div class="col-12">
                <label class="form-label fw-semibold">About Us Photo</label>
                <div class="d-flex align-items-center gap-3">
                  <div style="width:90px;height:90px;border-radius:12px;overflow:hidden;flex-shrink:0;background:#eee;display:flex;align-items:center;justify-content:center;">
                    <?php if (!empty($settings_page['about_image']) && file_exists('../' . $settings_page['about_image'])): ?>
                      <img src="../<?= htmlspecialchars($settings_page['about_image']) ?>" style="width:100%;height:100%;object-fit:cover;">
                    <?php else: ?>
                  ...
                  <div class="flex-grow-1">
                    ...
                    <div class="form-text">Shown in the "About Us" section on the website. JPG, PNG, GIF, or WEBP — under 5MB.</div>
                  </div>
                </div>
              </div>
              <div class="col-12">
...
/* ── Delete booking permanently ─────────────────────────── */
function confirmDeleteBooking(bookingId, name) {
  // Use the existing confirm modal if available, otherwise native confirm
  const modal = document.getElementById('deleteConfirmModal');
  if (modal) {
    document.getElementById('deleteConfirmName').textContent = name;
    document.getElementById('deleteConfirmId').value = bookingId;
    new bootstrap.Modal(modal).show();
  } else {
    if (confirm('Permanently delete booking for "' + name + '"?\nThis cannot be undone.')) {
      document.getElementById('deleteBookingForm-' + bookingId).submit();
    }
  }
}
  ...
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content border-0 shadow">
      ...
      <div class="modal-body text-center py-4">
        ...
        <p class="mb-1 fw-semibold">Permanently delete this booking?</p>
        <p class="text-muted small mb-0">
          "<span id="deleteConfirmName"></span>"<br>
      ...
      <div class="modal-footer border-0 pt-0 d-flex gap-2">
        ...
        <button type="button" class="btn btn-danger flex-fill fw-bold" onclick="doDeleteBooking()">
...
<script>
var _deleteBookingId = null;
document.getElementById('deleteConfirmModal').addEventListener('show.bs.modal', function() {
  // id already set via confirmDeleteBooking
});
function doDeleteBooking() {
  const id = document.getElementById('deleteConfirmId').value;
  const form = document.getElementById('deleteBookingForm-' + id);
  bootstrap.Modal.getInstance(document.getElementById('deleteConfirmModal')).hide();
  if (form) form.submit();
}
// Hidden input to pass id to doDeleteBooking
document.addEventListener('DOMContentLoaded', function() {
  var input = document.createElement('input');
  input.type = 'hidden';
  input.id   = 'deleteConfirmId';
