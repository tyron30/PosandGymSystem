...
if (isset($_POST['regenerate_qr'])) {
    ...
    $dir      = __DIR__ . '/../qr_codes';
    if (!is_dir($dir)) mkdir($dir, 0755, true);
    $filename = $new_token . '.png';
    $tmp_file = $dir . '/tmp_' . $new_token . '.png';
    $filepath = $dir . '/' . $filename;
    ...
    if (file_exists($tmp_file)) {
        ...
        $padding = 20;
        $total_w = $qr_w + $padding * 2;
        $total_h = $qr_h + $label_h + $padding * 2;
        $canvas  = imagecreatetruecolor($total_w, $total_h);
        $dark_teal = imagecolorallocate($canvas, 0, 80, 80);
        $white     = imagecolorallocate($canvas, 255, 255, 255);
        imagefill($canvas, 0, 0, $white);
    ...
    if ($upd->execute()) {
        header("Location: members.php?regen_success=1");
    } else {
...
if (isset($_POST['renew_member'])) {
    ...
    // Update member plan, start_date, end_date
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // fills the hidden fields, then submits the form.
            function autoGenerateQRAndSubmit(form, memberNameValue, endpoint) {
                ...
                fetch(endpoint, { method: 'POST', body: fd })
                    ...
                    .then(data => {
                        ...
                        // Inject token + path into the hidden fields
            ...
            // ── Add Member form (half month, monthly, etc.) ─────────────────────────
            ...
            if (addMemberForm) {
                ...
                addMemberForm.addEventListener('submit', function(e) {
                    // Only intercept when QR hasn't been pre-generated (hidden field is empty)
                    const tokenField = this.querySelector('[name="qr_token"]');
                    if (tokenField && tokenField.value.trim() !== '') return; // already have a token → normal submit
            ...
            if (quickAddForm) {
                // Ensure hidden fields exist (quick add didn't have them originally)
            ...
            // viewQR and confirmRegenQR are defined as global functions below
            ...
            window.printQR = function() {
                ...
                printWindow.document.write(`
                    ...
                    <style>
                        ...
                        .wrapper { display: inline-block; border: 2px solid #006666; border-radius: 12px; padding: 24px 32px; }
                        .gym-title { font-size: 11px; font-weight: bold; color: #006666; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 4px; }
    ...
    <script>
        // Global QR functions - must be outside DOMContentLoaded to work with onclick
