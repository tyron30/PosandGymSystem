...
if (isset($_POST['add_product'])) {
    ...
    if (isset($_FILES['product_image']) && !empty($_FILES['product_image']['name']) && $_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            // Create temp id later, use time+random to avoid collisions
            $new_filename = 'product_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
            $dest = $upload_dir . $new_filename;
            $moved = move_uploaded_file($_FILES['product_image']['tmp_name'], $dest);
            if (!$moved) {
                $moved = copy($_FILES['product_image']['tmp_name'], $dest);
            }
            if ($moved && file_exists($dest)) {
                $image_path = 'uploads/products/' . $new_filename;
            } else {
                $errors[] = "Failed to save image. Dir writable: " . (is_writable($upload_dir) ? 'yes' : 'no');
            }
        }
    } elseif (isset($_FILES['product_image']) && !empty($_FILES['product_image']['name']) && $_FILES['product_image']['error'] !== UPLOAD_ERR_NO_FILE) {
        $php_upload_errors = [
            UPLOAD_ERR_INI_SIZE   => 'File too large (php.ini limit)',
            UPLOAD_ERR_FORM_SIZE  => 'File too large (form limit)',
            UPLOAD_ERR_PARTIAL    => 'File only partially uploaded',
            UPLOAD_ERR_NO_TMP_DIR => 'No temp folder - check PHP upload_tmp_dir in php.ini',
            UPLOAD_ERR_CANT_WRITE => 'Cannot write to disk - check folder permissions',
            UPLOAD_ERR_EXTENSION  => 'Upload blocked by PHP extension',
        ];
        $err_code = $_FILES['product_image']['error'];
        $errors[] = $php_upload_errors[$err_code] ?? "Upload error code: $err_code";
    }
...
if (isset($_POST['edit_product'])) {
    ...
    } elseif (!empty($_FILES['product_image']['name']) && $_FILES['product_image']['error'] !== UPLOAD_ERR_NO_FILE) {
        $php_upload_errors = [
            UPLOAD_ERR_INI_SIZE   => 'File too large (php.ini limit)',
            UPLOAD_ERR_FORM_SIZE  => 'File too large (form limit)',
            UPLOAD_ERR_PARTIAL    => 'File only partially uploaded',
    ...
    // remove_image=1  → explicitly set NULL (user clicked Remove Image)
    // new file upload  → use new path (handled above, $image_path already set)
    // otherwise        → preserve existing path from current_image
    ...
    if (empty($errors)) {
        if ($remove_image) {
            // Explicitly clear the image column
            $null_val = null;
...
if (isset($_POST['delete_product'])) {
    ...
    // Soft-delete: set is_active = 0 so sales history stays intact
   ...
   <div class="d-flex">
                                        ...
                                        <div class="card pos-item h-100" style="cursor:pointer;transition:transform .15s,box-shadow .15s;" data-item-id="<?php echo $item['id']; ?>" data-item-name="<?php echo htmlspecialchars($item['name']); ?>" data-item-price="<?php echo $item['price']; ?>" data-stock="<?php echo $item['stock_quantity']; ?>">
                                            ...
                                            <div class="card-body text-center p-2">
                                                ...
                                                <div>
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-outline-primary edit-product-btn product-action-btn" data-item-id="<?php echo $item['id']; ?>" data-item-name="<?php echo htmlspecialchars($item['name']); ?>" data-item-price="<?php echo $item['price']; ?>" data-item-category="<?php echo $item['category']; ?>" data-item-stock="<?php echo $item['stock_quantity']; ?>" data-item-image="<?php echo htmlspecialchars($item['image'] ?? ''); ?>">
                                                        <i class="fas fa-edit"></i><span>Edit</span>
                                                    </button>
                                                    <button type="button" class="btn btn-sm btn-outline-danger delete-product-btn product-action-btn product-action-btn-icon" data-item-id="<?php echo $item['id']; ?>" data-item-name="<?php echo htmlspecialchars($item['name']); ?>" title="Delete product">
    ...
    <script>
        ...
        function showEditProductModal(btn) {
            ...
            preview.dataset.removeImage = '0'; // always reset removal flag on open
        ...
        // Clear image button — confirms, then marks image for removal on save
                    ...
                    onConfirm: function() {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                // Remove the card from the grid immediately without reload
        ...
        document.getElementById('editProductForm').addEventListener('submit', function(e) {
            ...
            // Send current image path so PHP can preserve it if no new file chosen
            const preview    = document.getElementById('editProductImagePreview');
