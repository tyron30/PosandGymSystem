...
.hero-bg {
  ...
  inset: 0;
  background:
    linear-gradient(135deg, rgba(13,13,13,0.92) 0%, rgba(13,13,13,0.6) 60%, rgba(47,128,237,0.08) 100%),
...
.program-card.featured {
  border-color: var(--gold);
  background: linear-gradient(145deg, var(--dark3), rgba(47,128,237,0.05));
}
.program-badge {
  position: absolute;
  top: 16px;
...
#fab {
  ...
  justify-content: center;
  font-size: 1.2rem;
  box-shadow: 0 8px 32px rgba(47,128,237,0.5);
  cursor: pointer;
  transition: transform var(--trans), box-shadow var(--trans);
  border: none;
  animation: fab-bounce 3s ease-in-out infinite;
}
#fab:hover { transform: scale(1.12); box-shadow: 0 12px 40px rgba(47,128,237,0.65); }
...
.plan-card-select:hover { border-color: rgba(47,128,237,0.4); background: rgba(47,128,237,0.04); }
.plan-card-select.selected { border-color: var(--gold); background: rgba(47,128,237,0.08); }
  ...
  <div class="modal-box">
    ...
    <div class="modal-body">
      ...
      <div id="booking-step-4" style="display:none;padding:0">
        <!-- Confirmation slip -->
        <div style="background:var(--dark4);border-radius:12px;padding:18px;margin-bottom:16px;border:1px solid rgba(47,128,237,.25)">
...
function siteAlert(message, type) {
  ...
  const styles = {
    ...
    info:    { icon: 'fa-info-circle',        bg: 'rgba(47,128,237,0.15)', color: 'var(--gold)', title: 'Notice' }
  };
